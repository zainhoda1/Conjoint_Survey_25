...
car_suv_lc_3c <- readRDS(here(
  ...
  "battery_analysis",
  "apollo",
  "nonlinear_rangeloss_car_suv_lc_3c_1_model.rds"
))
...
var_meta <- tribble(
  ...
  "range_year0"                , "BEV Electric Range (Year 3, 100 miles)"           , "WTP (*1000 USD) for Vehicle Attributes" , "dollar" ,
  "degradation"                , "Range Loss Rate (%)"                              , "WTP (*1000 USD) for Vehicle Attributes" , "dollar" ,
...
# sig has columns: Variables, class1, class2, class3
gt_formatted <- formatted %>%
  left_join(
    sig %>% filter(Variables %in% attributes),
...
write.xlsx(
  ...
  here::here(
    ...
    "battery_analysis",
    "apollo",
    "0_class_profile_summary_combined_nonlinear_lc_3c_excel.xlsx"
  )
)
...
gtsave(
  ...
  file = here::here(
    ...
    "battery_analysis",
    "apollo",
    "0_class_profile_summary_combined_nonlinear_lc_3c.html"
  )
)
