...
parent_meta <- parent_meta %>%
  group_by(col) %>% arrange(desc(n), .by_group = TRUE) %>% ungroup()

gas_id  <- "parent_gas_enthusiasm"
used_id <- "parent_used_distrust"
if (all(c(gas_id, used_id) %in% parent_meta$parent_id)) {
  same_col <- with(parent_meta, col[parent_id == gas_id] == col[parent_id == used_id])
  if (same_col) {
    ordering <- parent_meta$parent_id
    g <- which(ordering == gas_id); u <- which(ordering == used_id)
...
cat("Column heights (respondent-units): col1=", col_h[1],
    "  col2=", col_h[2], "\n", sep = "")
...
sub_layout <- subtheme_meta %>%
  ...
  left_join(parent_layout %>% select(parent_id, col, p_y_top = y_top, p_n = n),
            by = "parent_id") %>%
  group_by(parent_id) %>%
  mutate(
...
sub_layout_multi <- sub_layout_multi %>%
  left_join(parent_meta %>% select(parent_id, parent_color = color), by = "parent_id") %>%
  group_by(parent_id) %>%
  mutate(
    rank_in_parent = row_number(),
    lf = if (n() == 1) LIGHT_START
         else LIGHT_START + (rank_in_parent - 1) * (LIGHT_END - LIGHT_START) / (n() - 1),
    color = colorspace::lighten(parent_color, amount = lf, method = "relative", space = "HCL")
  ) %>%
  ungroup()
...
sub_y_ranges <- sub_layout_multi %>%
  ...
  summarise(y_top_mid = max(y_mid), y_bot_mid = min(y_mid), n_children = n(),
            .groups = "drop") %>%
  filter(n_children > 1)
...
jac <- co_mat
for (i in seq_along(active_subs)) for (j in seq_along(active_subs)) {
  if (i == j) { jac[i, j] <- NA; next }
...
for (i in seq_along(active_subs)) for (j in seq_along(active_subs)) {
  ...
  if (parent_of[a] == parent_of[b]) {
    if (joint < WITHIN_MIN) next
    within_pairs <- bind_rows(within_pairs, tibble(from = a, to = b, joint = joint))
  } else {
    if (joint < JOINT_MIN) next
    jacv <- jac[i, j]
    if (jacv < JACC_MIN) next
    cross_pairs <- bind_rows(cross_pairs,
                             tibble(from = a, to = b, joint = joint, jaccard = jacv))
  }
}
cat("Arcs to draw — cross-parent:", nrow(cross_pairs),
    "  within-parent:", nrow(within_pairs), "\n")
...
endpoint_lookup <- bind_rows(
  ...
  subtheme_meta %>% filter(parent_id %in% single_child_ids) %>%
    inner_join(parent_layout %>% select(parent_id, col, p_y_mid = y_mid),
               by = "parent_id") %>%
    mutate(x_inner = ifelse(col == 1, X_P1_RIGHT, X_P2_LEFT),
           y_mid   = p_y_mid) %>%
    select(subtheme, col, y_mid, x_inner)
)

arc_data <- all_edges %>%
  left_join(endpoint_lookup %>% rename(from = subtheme, col_a = col,
                                       y_a = y_mid, x_a = x_inner), by = "from") %>%
  left_join(endpoint_lookup %>% rename(to   = subtheme, col_b = col,
                                       y_b = y_mid, x_b = x_inner), by = "to") %>%
  left_join(subtheme_meta %>% select(from = subtheme, parent_a = parent_id),
            by = "from") %>%
  left_join(subtheme_meta %>% select(to = subtheme, parent_b = parent_id),
            by = "to") %>%
  mutate(
...
arc_data <- arc_data %>%
  ...
  mutate(
    ...
    apex_y = case_when(
      kind == "cross" ~ chord_y + (within_group_idx - (n_in_group + 1) / 2) *
                                    CROSS_STAGGER_STEP,
      TRUE            ~ chord_y
    )
  )
...
if (nrow(arc_data) > 0) {
  arc_paths <- do.call(rbind, lapply(seq_len(nrow(arc_data)), function(i) {
    r <- arc_data[i, ]
...
parent_layout <- parent_layout %>%
  mutate(lbl = paste0(parent_label, "\n(n=", n, " | ",
...
p <- ggplot() +
  # Parent themes: no color block, label only (see label layer below)
  # Sub-theme rectangles (multi-child parents only)
  geom_rect(data = sub_layout_multi,
            aes(xmin = x_left, xmax = x_right,
                ymin = y_bot, ymax = y_top, fill = color),
            color = "white", linewidth = 0.5) +
  # Bracket connectors
  geom_segment(data = seg_theme_to_bracket,
               aes(x = x, y = y, xend = xend, yend = yend),
               color = "black", linewidth = 0.7) +
  { if (nrow(bracket_verticals) > 0)
    geom_segment(data = bracket_verticals,
                 aes(x = x, y = y, xend = xend, yend = yend),
                 color = "black", linewidth = 0.7)
  } +
  geom_segment(data = seg_bracket_to_sub,
               aes(x = x, y = y, xend = xend, yend = yend),
               color = "black", linewidth = 0.7) +
  # Cross-mention arcs — bezier curves that pass exactly through their apex
  { if (nrow(arc_paths) > 0)
    geom_path(data = arc_paths,
              aes(x = x, y = y, group = arc_id),
              color = ARC_COLOR, linewidth = 0.7, linetype = "22", alpha = 0.85)
  } +
  # Badges sit exactly at each arc's apex point
  { if (nrow(arc_data) > 0) geom_label(
      data = arc_data,
  ...
  geom_text(data = parent_layout %>%
              mutate(lbl_y = ifelse(n < 8, y_bot + 0.3, y_mid),
...
cat(sprintf("Save size: %.1f x %.2f in (11:8.5 landscape aspect)\n", save_w, save_h))

out_dir <- here("code", "output", "images", "battery_analysis", "thematic_analysis")
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
png_path <- file.path(out_dir, "thematic_hierarchical_nobev.png")
svg_path <- file.path(out_dir, "thematic_hierarchical_nobev.svg")

ggsave(p, filename = png_path, width = save_w, height = save_h,
       dpi = 300, bg = "white", limitsize = FALSE)
ggsave(p, filename = svg_path, width = save_w, height = save_h,
