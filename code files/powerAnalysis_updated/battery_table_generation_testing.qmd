...
design_rand_output <- design_rand_output%>%
  mutate(image_refurbishment= case_when( 
    (no_choice == 0 & 
      battery_refurbishpackreplace == 0 &
      battery_refurbishcellreplace == 0) ~ paste("battery_survey_battery_original_text.png"),
    (no_choice == 0 & 
      battery_refurbishpackreplace == 1 &
      battery_refurbishcellreplace == 0) ~ paste("battery_survey_battery_packreplace_text.png"),
    (no_choice == 0 & 
      battery_refurbishpackreplace == 0 &
      battery_refurbishcellreplace == 1) ~ paste("battery_survey_battery_cellreplace_text.png") ,
    TRUE ~ 'NA'
      )
   )
...
#write.csv(design_random_fixed_parameter, here('code files', 'testing_initial_data_modeling', 'data_for_battery_image.csv'))
...
df_segmented <- df_range_SOH_long %>%
  mutate(
    ...
    fill_color = alpha("lightgrey", alpha = 0.6),
    visual_filled = ceiling(battery_health * segments),
    y_center = as.numeric(factor(year))
  ) %>%
  uncount(weights = segments, .id = "segment") %>%
...
for (id in row_ids) {
  ...
  # Filter data for this row_id
  segmented_i <- df_segmented %>% filter(row_id == id, year ==  year1)
  outline_i   <- df_battery_outline %>% filter(row_id == id, year ==  year1)
  terminal_i  <- df_terminal %>% filter(row_id == id, year ==  year1)
  label_i     <- df_range_SOH_long %>% filter(row_id == id, year == year1)
  ymin1 <- 1.7
  ymax1 <- 2.3
  ...
  #p1 <- 
    ...
    geom_rect(
      data = segmented_i,
      aes(xmin = xmin, xmax = xmax, ymin = ymin1, ymax = ymax1,
          fill = ifelse(filled, fill_color, "white")),
      color = NA
    ) +
    geom_rect(
    ...
    geom_rect(
      data = terminal_i,
      aes(xmin = xmin, xmax = xmax, ymin = ymin1+0.1, ymax = ymax1-0.1),
      fill = "black"
    ) +
    scale_fill_identity() +
    coord_cartesian(xlim = c(0, max(df_range_SOH_long$battery_range_max)),  clip = "off") +
