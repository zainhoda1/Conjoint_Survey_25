...
# UPLOAD MODELS ----
car_suv_lc_8c <- readRDS(here(
  "code", "output", "model_output", "battery_analysis", "apollo",
  "piecewise_rangeloss_car_suv_lc_8c_1_model.rds"
))
car_suv_lc_8c_apollo_inputs <- readRDS(here(
  "code", "output", "model_output", "battery_analysis", "apollo",
  "car_suv_lc_8c_apollo_inputs_piecewise_rangeloss.rds"
))
car_suv_lc_8c_apollo_probabilities <- readRDS(here(
  "code", "output", "model_output", "battery_analysis", "apollo",
  "car_suv_lc_8c_apollo_probabilities_piecewise_rangeloss.rds"
))
...
parse_class_suffix <- function(x) {
  case_when(
    x == "a" ~ "class1", x == "b" ~ "class2", x == "c" ~ "class3",
    x == "d" ~ "class4", x == "e" ~ "class5", x == "f" ~ "class6",
    x == "g" ~ "class7", TRUE ~ "class8"
  )
}

estimates_8c <- read_csv(here(
  "code", "output", "model_output", "battery_analysis", "apollo",
  "piecewise_rangeloss_car_suv_lc_8c_1_estimates.csv"
))
colnames(estimates_8c)[1] <- "variable"

estimates_long <- estimates_8c %>%
  as.data.frame() %>% select(1:2, 6, 8) %>%
  setNames(c("Variables","Est.","Std.Err.","P.Value")) %>%
  filter(!is.na(Std.Err.)) %>%
  mutate(Variables = str_replace_all(Variables, "b_|gamma_", ""),
         Variables = str_replace_all(Variables, "delta", "ASC")) %>%
  separate(col=Variables, into=c("Variables","Class"), sep="_(?=[^_]+$)", extra="merge", fill="right") %>%
  mutate(Class = parse_class_suffix(Class))

price_pval_by_class <- estimates_long %>% filter(Variables=="price") %>% select(Class, price_pval=P.Value)
...
wtp_sig <- estimates_long %>%
  ...
  mutate(WTP_pval=pmax(P.Value, price_pval, na.rm=TRUE),
    WTP_sig=case_when(WTP_pval<0.001~"***",WTP_pval<0.01~"**",WTP_pval<0.05~"*",WTP_pval<0.1~".",TRUE~" ")) %>%
...
summarize_lc_model <- function(model, apollo_probabilities, apollo_inputs,
  ...
  db_indiv <- db %>%
    ...
    relocate(prob_class1,prob_class2,prob_class3,prob_class4,
             prob_class5,prob_class6,prob_class7,prob_class8, .after=last_col()) %>%
    mutate(
      ...
      prob_class_assign = case_when(
        prob_class1==prob_class_max~"class1", prob_class2==prob_class_max~"class2",
        prob_class3==prob_class_max~"class3", prob_class4==prob_class_max~"class4",
        prob_class5==prob_class_max~"class5", prob_class6==prob_class_max~"class6",
        prob_class7==prob_class_max~"class7", prob_class8==prob_class_max~"class8",
        TRUE~NA_character_
      )
    )
...
combined_res <- summarize_lc_model(
  car_suv_lc_8c, car_suv_lc_8c_apollo_probabilities, car_suv_lc_8c_apollo_inputs,
...
formatted <- combined_all %>%
  rowwise() %>%
  mutate(fmt=case_when(variable %in% attributes_long~"dollar",grepl("_share$",variable)~"pct",grepl("^optout_",variable)~"pct",TRUE~"number"),
...
gt_formatted <- formatted %>%
  ...
  mutate(
    ...
    fmt_Combined_class5 = paste0(fmt_Combined_class5,"\n",coalesce(class5,"")),
    fmt_Combined_class6 = paste0(fmt_Combined_class6,"\n",coalesce(class6,"")),
    fmt_Combined_class7 = paste0(fmt_Combined_class7,"\n",coalesce(class7,"")),
    fmt_Combined_class8 = paste0(fmt_Combined_class8,"\n",coalesce(class8,""))
  )
...
gtsave(gt_car_suv_lc_8c,
  file=here::here("code","output","model_output","battery_analysis","apollo",
                  "0_class_profile_summary_combined_lc_8c_rangeloss.html"))

# ── Class 8 Subgroup Analysis ─────────────────────────────────────────────────
library(glue); library(lubridate)

class_probs_8c <- read_parquet(here(
  "code","output","model_output","battery_analysis","apollo",
  "0_Combined_8c_class_probabilities.parquet"
...
c8_resp <- class_probs_8c %>%
  filter(prob_class_assign=="class8") %>%
  left_join(timing_c8,by="psid") %>% left_join(optout_per_resp,by="psid") %>%
  left_join(resp_covariates,by="psid") %>% filter(!is.na(mean_cbc_sec))

median_sec <- median(c8_resp$mean_cbc_sec, na.rm=TRUE)
c8_resp <- c8_resp %>%
  mutate(speed_group=factor(if_else(mean_cbc_sec<=median_sec,"Fast","Slow"),levels=c("Fast","Slow")))

diag_summary <- c8_resp %>% group_by(speed_group) %>%
  summarise(n=n(), mean_duration_sec=weighted.mean(mean_cbc_sec,prob_class8,na.rm=TRUE),
            optout_rate=weighted.mean(optout_rate_resp,prob_class8,na.rm=TRUE),
            optout_never_pct=weighted.mean(optout_never,prob_class8,na.rm=TRUE), .groups="drop")

compare_num <- map_dfr(num_vars, function(v) {
  map_dfr(c("Fast","Slow"), function(grp) {
    sub <- c8_resp %>% filter(speed_group==grp)
    tibble(variable=v,speed_group=grp,value=weighted.mean(sub[[v]],sub$prob_class8,na.rm=TRUE))
  })
...
compare_cate <- map_dfr(binary_cate_vars, function(v) {
  map_dfr(c("Fast","Slow"), function(grp) {
    ...
    tibble(variable=v,speed_group=grp,
           value=weighted.mean(as.numeric(sub[[v]]==1),sub$prob_class8,na.rm=TRUE))
  })
}) %>% pivot_wider(names_from=speed_group, values_from=value)
...
gt_c8_subgroup <- display_tbl %>% group_by(section) %>% gt(rowname_col="label") %>%
  tab_header(title=md("**Class 8 Internal Subgroup Comparison: Fast vs. Slow Responders**"),
  ...
  tab_footnote(footnote="All means probability-weighted by each respondent's posterior P(class 8).",locations=cells_title("subtitle")) %>%
...
gtsave(gt_c8_subgroup,
  file=here::here("code","output","model_output","battery_analysis","apollo","0_c8_subgroup_fast_vs_slow.html"))

p_c8_scatter <- c8_resp %>%
  mutate(mean_cbc_cap=pmin(mean_cbc_sec,quantile(mean_cbc_sec,0.97,na.rm=TRUE))) %>%
  ggplot(aes(x=mean_cbc_cap,y=optout_rate_resp,color=prob_class8,size=prob_class8)) +
  ...
  scale_color_gradient(low="#FFCDD2",high="#B71C1C",name="P(class 8)") +
  scale_size_continuous(range=c(1,4),guide="none") +
  scale_y_continuous(labels=scales::percent,breaks=seq(0,1,0.2)) +
  labs(title="C8 Respondents: Task Duration vs. Opt-Out Rate",
...
ggsave(here("code","output","images","battery_analysis","c8_duration_vs_optout_scatter.png"),
       p_c8_scatter,width=9,height=5.5,dpi=300)
