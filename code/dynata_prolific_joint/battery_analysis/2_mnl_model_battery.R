...
## model estimation ----
# model_car <- run_model1(data_dce %>% filter(vehicle_typesuv == 0))
# model_suv <- run_model1(data_dce %>% filter(vehicle_typesuv == 1))
model1_all <- run_model1(data_dce)
...
model2_car <- run_model2(data_dce %>% filter(vehicle_typesuv == 0))
model2_suv <- run_model2(data_dce %>% filter(vehicle_typesuv == 1))
model2_all <- run_model2(data_dce)

# Estimate the wtp model
model3_car <- run_model_wtp(data_dce %>% filter(vehicle_typesuv == 0))
model3_suv <- run_model_wtp(data_dce %>% filter(vehicle_typesuv == 1))
model3_all <- run_model_wtp(data_dce)
