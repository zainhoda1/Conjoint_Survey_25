...
formatted <- coef_tbl %>%
  separate(
    col = tag,
    into = c("Information", "Segment", "Class"),
    sep = "_",
    extra = "merge",
    fill = "right"
  ) %>%
  pivot_wider(
    id_cols = c(variable),
    names_from = c(Information, Segment, Class),
    values_from = c(estimate, std_error, sig),
    names_glue = "{.value}_{Information}_{Segment}_{Class}"
  )
...
gt_formatted <- formatted %>%
  as.data.frame() %>%
  mutate(
    treat0_car_class1 = glue(
      "{estimate_treat0_car_class1}\n({std_error_treat0_car_class1}){sig_treat0_car_class1}",
      .na = NULL
    ),
    treat0_car_class2 = glue(
      "{estimate_treat0_car_class2}\n({std_error_treat0_car_class2}){sig_treat0_car_class2}",
      .na = NULL
    ),
    treat0_suv_class1 = glue(
      "{estimate_treat0_suv_class1}\n({std_error_treat0_suv_class1}){sig_treat0_suv_class1}",
      .na = NULL
    ),
    treat0_suv_class2 = glue(
      "{estimate_treat0_suv_class2}\n({std_error_treat0_suv_class2}){sig_treat0_suv_class2}",
      .na = NULL
    ),
    treat1_car_class1 = glue(
      "{estimate_treat1_car_class1}\n({std_error_treat1_car_class1}){sig_treat1_car_class1}",
      .na = NULL
    ),
    treat1_car_class2 = glue(
      "{estimate_treat1_car_class2}\n({std_error_treat1_car_class2}){sig_treat1_car_class2}",
      .na = NULL
    ),
    treat1_suv_class1 = glue(
      "{estimate_treat1_suv_class1}\n({std_error_treat1_suv_class1}){sig_treat1_suv_class1}",
      .na = NULL
    ),
    treat1_suv_class2 = glue(
      "{estimate_treat1_suv_class2}\n({std_error_treat1_suv_class2}){sig_treat1_suv_class2}",
      .na = NULL
    )
  ) %>%
  select(
    label,
    section,
    starts_with("treat0_car"),
    starts_with("treat0_suv"),
    starts_with("treat1_car"),
    starts_with("treat1_suv")
  ) %>%
...
summarize_class_size <- ftable(
  ...
  group_by(vehicle_typesuv, battery_info_treat) %>%
  mutate(mean_probability = round(class_size / sum(class_size), 3)) %>%
  ungroup() %>%
  arrange(battery_info_treat, vehicle_typesuv, class_name) %>%
  mutate(
...
gt_car_suv_ml_2c <- gt_formatted |>
  ...
  tab_spanner(
    label = "SUV",
    id = "treat0_suv",
    columns = starts_with("treat0") & contains("suv")
  ) |>
  tab_spanner(
    label = "Car",
    id = "treat1_car",
    columns = starts_with("treat1") & contains("car")
  ) |>
  tab_spanner(
    label = "SUV",
    id = "treat1_suv",
    columns = starts_with("treat1") & contains("suv")
  ) |>
  tab_spanner(
    label = "No Warranty Information Treatment",
    columns = starts_with("treat0")
  ) |>
  tab_spanner(
    label = "Warranty Information Treatment",
    columns = starts_with("treat1")
  ) |>
...
gtsave(
  ...
  file = here::here(
    ...
    "logitr",
    "latent_class",
    "0_model_summary_cars_suvs_ml_2c_infotreat.html"
  )
)
