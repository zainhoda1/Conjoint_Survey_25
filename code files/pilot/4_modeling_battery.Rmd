...
data_DCE_wider<-data_DCE %>%
  pivot_wider(
  names_from = altID,
  values_from = c(veh_mileage,veh_price,
                  battery_range_year0,battery_degradation,
                  battery_refurbish)
  )
  select(battery_respID,)
  
  
data_DCE_wider<-data_DCE %>%
  select(-c(profileID,no_choice)) %>% 
  pivot_wider(
    id_cols = c(battery_respID,qID,obsID,respID),
    names_from = altID,
    values_from = c(veh_mileage, veh_price,battery_range_year0,battery_degradation,
                  battery_refurbish,choice),
    names_glue = "{.value}_{altID}"
  ) %>%
  mutate(choice = max.col(select(., starts_with("choice_")), ties.method = "first"))
