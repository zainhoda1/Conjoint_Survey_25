...
### Car+SUV
apollo_control = list(
  modelName = paste0("piecewise_rangeloss_car_suv_lc_8c_", i),
  modelDescr = "LC model with 8 classes, piecewise linear range and range loss",
  indivID = "respID",
  nCores = 6,
  panelData = TRUE,
...
apollo_beta = c(
  ...
  # Class H is the reference (fixed to 0)
  delta_a = 0,
  ...
  delta_f = 0,
  gamma_EV_rangeanxiety_f = 0,
  gamma_risktaker_f = 0,
  gamma_hhincome_f = 0,
  gamma_ev_charge_f = 0,
  # gamma_ev_neighbor_f = 0,
  gamma_knowledge_ev_f = 0,
  # gamma_knowledge_subsidy_f = 0,
  gamma_evb_environment_agree_f = 0,
  gamma_evb_function_disagree_f = 0,
  gamma_hhveh_fuel_f = 0,
  # gamma_veh_refuel_f = 0,
  gamma_veh_range_f = 0,
  gamma_suv_f = 0,

  delta_g = 0,
  gamma_EV_rangeanxiety_g = 0,
  gamma_risktaker_g = 0,
  gamma_hhincome_g = 0,
  gamma_ev_charge_g = 0,
  # gamma_ev_neighbor_g = 0,
  gamma_knowledge_ev_g = 0,
  # gamma_knowledge_subsidy_g = 0,
  gamma_evb_environment_agree_g = 0,
  gamma_evb_function_disagree_g = 0,
  gamma_hhveh_fuel_g = 0,
  # gamma_veh_refuel_g = 0,
  gamma_veh_range_g = 0,
  gamma_suv_g = 0,

  delta_h = 0,
  gamma_EV_rangeanxiety_h = 0,
  gamma_risktaker_h = 0,
  gamma_hhincome_h = 0,
  gamma_ev_charge_h = 0,
  # gamma_ev_neighbor_h = 0,
  gamma_knowledge_ev_h = 0,
  # gamma_knowledge_subsidy_h = 0,
  gamma_evb_environment_agree_h = 0,
  gamma_evb_function_disagree_h = 0,
  gamma_hhveh_fuel_h = 0,
  # gamma_veh_refuel_h = 0,
  gamma_veh_range_h = 0,
  gamma_suv_h = 0
)

### Class H is the reference class: fix all its allocation parameters to 0
apollo_fixed = c(
  "delta_a",
...
apollo_beta = apollo_readBeta(
  apollo_beta,
  apollo_fixed,
  "piecewise_rangeloss_car_suv_lc_7c_1",
...
apollo_lcPars = function(apollo_beta, apollo_inputs) {
  ...
  V[["class_f"]] = delta_f +
    gamma_EV_rangeanxiety_f * (ATT_range_anxiety > 3) +
    gamma_risktaker_f * (ATT_risktaker > 3) +
    gamma_hhincome_f * hhincome_num_10k +
    gamma_ev_charge_f * (EV_charger == "yes") +
    # gamma_ev_neighbor_f * (EV_neighbor == "yes") +
    gamma_knowledge_ev_f * (knowledge_ev == 1) +
    # gamma_knowledge_subsidy_f * (knowledge_subsidy == 1) +
    gamma_evb_environment_agree_f * (ATT_EVB_environment > 3) +
    gamma_evb_function_disagree_f * (ATT_EVB_function < 3) +
    gamma_hhveh_fuel_f * (Veh_hh_fuel == "icev_only") +
    # gamma_veh_refuel_f * Veh_primary_refuel_monthly +
    gamma_veh_range_f * (Veh_primary_range / 100) +
    gamma_suv_f * (vehicle_typesuv == 1)

  V[["class_g"]] = delta_g +
    gamma_EV_rangeanxiety_g * (ATT_range_anxiety > 3) +
    gamma_risktaker_g * (ATT_risktaker > 3) +
    gamma_hhincome_g * hhincome_num_10k +
    gamma_ev_charge_g * (EV_charger == "yes") +
    # gamma_ev_neighbor_g * (EV_neighbor == "yes") +
    gamma_knowledge_ev_g * (knowledge_ev == 1) +
    # gamma_knowledge_subsidy_g * (knowledge_subsidy == 1) +
    gamma_evb_environment_agree_g * (ATT_EVB_environment > 3) +
    gamma_evb_function_disagree_g * (ATT_EVB_function < 3) +
    gamma_hhveh_fuel_g * (Veh_hh_fuel == "icev_only") +
    # gamma_veh_refuel_g * Veh_primary_refuel_monthly +
    gamma_veh_range_g * (Veh_primary_range / 100) +
    gamma_suv_g * (vehicle_typesuv == 1)

  V[["class_h"]] = delta_h +
    gamma_EV_rangeanxiety_h * (ATT_range_anxiety > 3) +
    gamma_risktaker_h * (ATT_risktaker > 3) +
    gamma_hhincome_h * hhincome_num_10k +
    gamma_ev_charge_h * (EV_charger == "yes") +
    # gamma_ev_neighbor_h * (EV_neighbor == "yes") +
    gamma_knowledge_ev_h * (knowledge_ev == 1) +
    # gamma_knowledge_subsidy_h * (knowledge_subsidy == 1) +
    gamma_evb_environment_agree_h * (ATT_EVB_environment > 3) +
    gamma_evb_function_disagree_h * (ATT_EVB_function < 3) +
    gamma_hhveh_fuel_h * (Veh_hh_fuel == "icev_only") +
    # gamma_veh_refuel_h * Veh_primary_refuel_monthly +
    gamma_veh_range_h * (Veh_primary_range / 100) +
    gamma_suv_h * (vehicle_typesuv == 1)
