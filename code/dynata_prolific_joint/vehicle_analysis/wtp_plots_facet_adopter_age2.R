# wtp_plots_facet_adopter_age2.R
#
# Faceted dumbbell plot of BEV WTP by adoption propensity.
# Vehicle age fixed at 2 years; driving range (100, 200, 300 miles) shown as colors.
# facet_wrap by vehicle type (car / SUV).
#
# Mirrors Figure 3 (dumbbell_wtp_age_fixed_age_2_adopters.png) in facet layout.
...
plot_wide <- plot_long %>%
  ...
  filter(
    vehicle_type  == "BEV",
    current_age   == 2,              # age fixed at 2 years
    current_range %in% c(1, 2, 3)   # 100, 200, 300 miles
  ) %>%
  select(model_type, segment, current_range, wtp_total, wtp_value_type) %>%
  pivot_wider(names_from = wtp_value_type, values_from = wtp_total) %>%
  mutate(
    current_range = factor(current_range, levels = c(1, 2, 3),
                           labels = c("100 miles", "200 miles", "300 miles")),
  ...
  mutate(
    ...
    range_offset = case_when(
      current_range == "100 miles" ~ -0.22,
      current_range == "200 miles" ~  0.00,
      current_range == "300 miles" ~  0.22
    ),
    y_pos = seg_num + range_offset
  )

# ---- Plot --------------------------------------------------------------
ggplot(plot_wide, aes(y = y_pos, colour = current_range)) +
  geom_segment(
  ...
  labs(
    ...
    subtitle = "Vehicle age fixed at 2 years. Points: mean estimate; bars: 95% CI.",
...
cat("Saved: facet_wtp_adopter_age2.png\n")
