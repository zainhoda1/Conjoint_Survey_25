...
model <- model_positive_group_car
...
conf_model_positive_group_car <- get_cis(get_wtp_draws(model_positive_group_car))
conf_model_negative_group_car <- get_cis(get_wtp_draws(model_negative_group_car))
conf_model_positive_group_suv <- get_cis(get_wtp_draws(model_positive_group_suv))
conf_model_negative_group_suv <- get_cis(get_wtp_draws(model_negative_group_suv))


predict(model_negative_group_car)




all_models <- c("conf_model_positive_group_car",
...
temp %>%  
  mutate(
    model_type = case_when(
      model_name %in% c('conf_model_positive_group_car', 'conf_model_negative_group_car') ~ 'car',
      .default = 'suv'
    ),
    segment = case_when(
      model_name %in% c('conf_model_positive_group_car', 'conf_model_positive_group_suv') ~ 'Positive Group',
      .default = 'Negative Group'
    )
  ) %>% 
  filter(
    vehicle_type == "BEV",
    ...
    #(
     # (segment == 'Positive Group' & current_range < 2.5) |
     #   (segment == 'Negative Group' & current_range > 1.5)
    #)
  ) %>% 
  mutate(
 ...
 # scale_color_brewer(palette = "Blues", name = "Vehicle age\n(years)") +
  ...
  scale_linetype_manual(
    values = c("Positive Group" = "dashed", "Negative Group" = "solid"),
    name = "Model segment"
  ) +
...
ggsave(
  filename = here::here(
    ...
    "images",
    "vehicle_analysis",
    "vehicle_wtp_range_pos_neg_groups.png"    
  ),
  width = 8,
  height = 6,
  dpi = 300
...
ggsave(
  filename = here::here(
    ...
    "images",
    "vehicle_analysis",
    "wtp_range_with_coef_int_post_neg_groups.png"    
  ),
  width = 8,
  height = 6,
  dpi = 300
...
temp4 |> 
  mutate(segment = case_when(
    segment == 'Positive Group' ~ 'Low Budget',
    .default = 'High Budget'),
    model_type = case_when (
      model_type == 'car'  ~ 'CAR',
      .default = 'SUV'
...
ggsave(
  filename = here::here(
    ...
    "images",
    "vehicle_analysis",
    "wtp_range_with_coef_int_pos_neg.png"    
  ),
  width = 8,
  height = 6,
  dpi = 300
