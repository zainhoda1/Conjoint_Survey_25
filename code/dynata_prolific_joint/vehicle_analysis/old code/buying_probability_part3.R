...
for (j in c('Likely BEV Adopter', 'Unlikely BEV Adopter')){
  print(j)
...
for (i in seq(nrow(vehicle_pairs))){
  ...
  current_model = case_when(
    vehicle_pairs[i,3] == 'car'  & j == 'Likely BEV Adopter' ~ 'model_likely_bev_adopter_car',
    vehicle_pairs[i,3] == 'car'  & j == 'Unlikely BEV Adopter' ~ 'model_unlikely_bev_adopter_car',
    vehicle_pairs[i,3] == 'suv'  & j == 'Likely BEV Adopter' ~ 'model_likely_bev_adopter_suv',
    .default = 'model_unlikely_bev_adopter_suv'
  )
  #i = 1
