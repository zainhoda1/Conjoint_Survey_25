...
# UPLOAD MODELS ----
car_suv_lc_2c <- readRDS(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
  "apollo",
  "piecewise_rangeloss_car_suv_lc_2c_1_model.rds"
))

car_suv_lc_2c_apollo_inputs <- readRDS(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
...
car_suv_lc_2c_apollo_probabilities <- readRDS(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
...
estimates_2c <- read_csv(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
  "apollo",
  "piecewise_rangeloss_car_suv_lc_2c_1_estimates.csv"
))
colnames(estimates_2c)[1] <- "variable"

estimates_long <- estimates_2c %>%
  as.data.frame() %>%
  select(1:2, 6, 8) %>%
...
combined_res <- summarize_lc_model(
  car_suv_lc_2c,
  car_suv_lc_2c_apollo_probabilities,
  car_suv_lc_2c_apollo_inputs,
  database,
  "Combined",
  attributes,
  num_vars,
...
gtsave(
  ...
  file = here::here(
    ...
    "battery_analysis",
    "apollo",
    "0_class_profile_summary_combined_lc_2c_rangeloss.html"
  )
)
...
compare_cate <- map_dfr(binary_cate_vars, function(v) {
  map_dfr(c("Fast", "Slow"), function(grp) {
    ...
    tibble(
      variable    = v,
      speed_group = grp,
      value       = weighted.mean(as.numeric(sub[[v]] == 1), sub$prob_class2, na.rm = TRUE)
    )
  })
}) %>%
  pivot_wider(names_from = speed_group, values_from = value)
...
gt_c2_subgroup <- display_tbl %>%
  ...
  gt(rowname_col = "label") %>%
  tab_header(
    title    = md("**Class 2 Internal Subgroup Comparison: Fast vs. Slow Responders**"),
    subtitle = md(glue(
