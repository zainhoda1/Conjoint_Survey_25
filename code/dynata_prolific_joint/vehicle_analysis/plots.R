...
temp <- temp %>%  
 ...
 to_use <- temp %>%
   filter(
     !model_name %in% c('conf_model_car_high', 'conf_model_suv_high'),   # remove the "!" for high
     bev_type %in% c('BEV100', 'BEV200')   #  'BEV200', 'BEV300' for high
          )
 
 wtp_min <- min(to_use$WTP_BEV)
 wtp_max <- max(to_use$WTP_BEV)

# breaks = scales::breaks_extended(n = 3)(c(wtp_min, wtp_max))
 ...
 to_use %>%
   ...
   labs(
     ...
     title = 'Low budget models (Vehicle Range 100-200 miles)'  # High budget models (Vehicle Range 200-300 miles)
   ) +
 ...
 ggsave(
   filename = here::here(
     ...
     'output',
     "images",
     "vehicle_wtp_low_budget.png"     #high_budget
   ),
   width = 8,
   height = 6,
   dpi = 300
