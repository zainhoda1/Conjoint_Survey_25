...
priors_fixed <- cbc_priors(
  ...
  battery_range_year0  = 0.01,         # Each 10 mile of range adds utility by 0.1
  battery_degradation  = -10          # Each 1% of degradation increases subtracts utility by 0.1
...
priors_random_parameter <- cbc_priors(
  profiles = profiles,
  veh_mileage          = -0.00005,     # Each 1000 mile increase reduces utility by 0.05
  veh_price = rand_spec("n", -0.0001, 0.00005), # Assume $1,000 price increase = -0.1 utility
  battery_range_year0 = rand_spec("n", 0.01, 0.005), # Assume each extra mile = +0.01 utility
  battery_degradation = rand_spec("n", -10, 3), # 1% increase in degradation = -0.10 utility
...
priors_random_parameter_ln <- cbc_priors(
  profiles = profiles,
  veh_mileage          = -0.00005,     # Each 1000 mile increase reduces utility by 0.05
  veh_price = rand_spec("ln", log(0.0001), 0.5), # Assume $1,000 price increase = -0.1 utility
  battery_range_year0 = rand_spec("n", 0.01, 0.005), # Assume each extra mile = +0.01 utility
  battery_degradation = rand_spec("ln", log(10), 0.3), # 1% increase in degradation = -0.10 utility
  battery_refurbish = rand_spec("n", c(-1.0, -0.5), c(0.1, 0.1)) # Reference = "original"
)

# draws <- draw_parameters(priors_ln, n_draws = 1000)
# draws$veh_price <- -1 * draws$veh_price
# draws$battery_degradation <- -1 * draws$battery_degradation

priors_random_parameter_ln
```
...
design_stochastic_random_parameter_ln <- cbc_design(
  profiles = profiles,
  method = "stochastic",
  n_resp   = 3000, # Number of respondents
  n_alts   = 3,    # Number of alternatives per question
  n_q      = 6,    # Number of questions per respondent #6
  priors = priors_random_parameter_ln,
  n_start = 1
) 
...
```{r}
cbc_inspect(design_stochastic_fixed_parameter)
```

```{r}
cbc_inspect(design_stochastic_random_parameter)
```

# Step 5: Simulate Choices

```{r}
choices_random <- cbc_choices(design_random)
choices_fixed_parameter <- cbc_choices(design_stochastic_fixed_parameter, priors = priors_fixed)
choices_random_parameter <- cbc_choices(design_stochastic_random_parameter, priors = priors_random_parameter)
...
## Random
```{r}
choices_cat <- cbc_decode(choices_random)
...
## Stochastic- Fixed parameter
```{r}
choices_cat <- cbc_decode(choices_fixed_parameter)
...
## Stochastic- Random parameter
```{r}
choices_cat <- cbc_decode(choices_random_parameter)
...
plot(power_random, type = "power", power_threshold = 0.9)

```

```{r}
power_fixed_parameter <- cbc_power(choices_fixed_parameter)
plot(power_fixed_parameter, type = "power", power_threshold = 0.9)
```

```{r}
power_random_parameter <- cbc_power(choices_random_parameter)
plot(power_random_parameter, type = "power", power_threshold = 0.9)
```
<!-- ## Output -->
<!-- ```{r} -->
