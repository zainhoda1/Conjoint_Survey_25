...
class_labels <- c(
  class1 = "C1: Hostile Reluctant",
  class2 = "C2: Refurbishment Averse",
  class3 = "C3: EV-open, Range Focused",
  class4 = "C4: Budget-constrained",
  class5 = "C5: Attribute Insensitive"
)

# Posterior class probabilities per respondent (model sample only)
class_probs <- class_prop %>%
  select(psid, prob_class1, prob_class2, prob_class3, prob_class4, prob_class5)
...
gtsave(
  ...
  file = here::here(
    ...
    "battery_analysis",
    "apollo",
    "0_optout_rate_by_class_5c.html"
  )
)
...
gtsave(
  ...
  file = here::here(
    ...
    "battery_analysis",
    "apollo",
    "0_survey_duration_by_class_5c.html"
  )
)

# # ---- Class 5: Duration Distribution (Bimodality Check) ----

# Hard-assigned class 5 respondents, with their posterior prob_class5 as alpha
c5_time <- data_time %>%
  inner_join(
    class_prop %>% select(psid, prob_class5, prob_class_assign),
    by = "psid"
  ) %>%
  filter(prob_class_assign == "class5")

# Reference: hard-assigned class 3 (engaged, rational class)
c3_time <- data_time %>%
  inner_join(
...
c5_long <- c5_time %>%
  ...
  mutate(
    Class = "C5: Attribute Insensitive",
    # Cap at 99th percentile to suppress extreme outliers in display
...
c3_long <- c3_time %>%
  ...
  filter(!is.na(seconds), seconds > 0) %>%
  mutate(
    Class = "C3: EV-open, Range Focused",
    seconds_cap = pmin(seconds, quantile(seconds, 0.90, na.rm = TRUE))
  )

# --- Plot 1: C5 density per question, with rug to expose fast clickers ---
p_c5_dist <- c5_long %>%
  ...
  labs(
    title = "C5 (Attribute Insensitive): Decision Time Distribution per Choice Task",
    subtitle = "Hard-assigned class 5 respondents. Rug marks individual observations. X capped at 90th pct.",
...
# # --- Plot 2: C3 vs C5 overlaid density per question ---
# p_c3_c5 <- bind_rows(c3_long, c5_long) %>%
#   ggplot(aes(x = seconds_cap, fill = Class, color = Class)) +
#   geom_density(alpha = 0.35, linewidth = 0.8) +
#   facet_wrap(~Question, ncol = 3) +
#   scale_x_continuous(breaks = seq(0, 120, 20)) +
#   scale_fill_manual(values = c(
#     "C3: EV-open, Range Focused" = "#1565C0",
#     "C5: Attribute Insensitive" = "#E53935"
#   )) +
...
#   )) +
#   labs(
#     title = "Decision Time Distribution: C3 vs C5 by Choice Task",
#     subtitle = "Hard-assigned respondents. X capped at 99th pct. Faster C5 peak = satisficing signal.",
#     x = "Seconds on choice task",
#     y = "Density",
#     fill = NULL,
#     color = NULL
...
# ggsave(
#   here("code", "output", "images", "battery_analysis", "c3_c5_duration_comparison.png"),
#   p_c3_c5,
#   width = 10, height = 6, dpi = 300
# )

# # --- Plot 3: C5 respondents sorted by mean task duration ---
# # Reveals whether fast and slow C5 respondents differ systematically
# c5_resp_mean <- c5_long %>%
#   group_by(psid, prob_class5) %>%
#   summarise(mean_sec = mean(seconds, na.rm = TRUE), .groups = "drop") %>%
#   arrange(mean_sec) %>%
#   mutate(rank = row_number())

# p_c5_sorted <- c5_resp_mean %>%
#   ggplot(aes(x = rank, y = mean_sec, color = prob_class5)) +
#   geom_point(size = 1.8, alpha = 0.8) +
#   geom_hline(yintercept = 10, linetype = "dashed", color = "firebrick", linewidth = 0.7) +
#   annotate(
#     "text",
#     x = 5, y = 12,
#     label = "10 sec threshold",
...
#   labs(
#     title = "C5 Respondents Ranked by Mean Task Duration",
#     subtitle = "Each point = one respondent. Colored by posterior class 5 probability.",
#     x = "Rank (fastest to slowest)",
#     y = "Mean seconds per task"
#   ) +
#   theme_minimal(base_family = "Roboto Condensed") +
#   theme(
#     plot.title = element_text(face = "bold", size = 13),
...
# ggsave(
#   here("code", "output", "images", "battery_analysis", "c5_respondent_duration_rank.png"),
#   p_c5_sorted,
