...
# UPLOAD MODELS ----
car_suv_lc_7c <- readRDS(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
  "apollo",
  "piecewise_rangeloss_car_suv_lc_7c_1_model.rds"
))

car_suv_lc_7c_apollo_inputs <- readRDS(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
  "apollo",
  "car_suv_lc_7c_apollo_inputs_piecewise_rangeloss.rds"
))

car_suv_lc_7c_apollo_probabilities <- readRDS(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
  "apollo",
  "car_suv_lc_7c_apollo_probabilities_piecewise_rangeloss.rds"
))
...
estimates_7c <- read_csv(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
  "apollo",
  "piecewise_rangeloss_car_suv_lc_7c_1_estimates.csv"
))
colnames(estimates_7c)[1] <- "variable"

estimates_long <- estimates_7c %>%
  as.data.frame() %>%
  select(1:2, 6, 8) %>%
  ...
  db_indiv <- db %>%
    ...
    mutate(
      ...
      prob_class_assign = case_when(
        ...
        prob_class7 == prob_class_max ~ "class7",
        TRUE ~ NA_character_
      )
    )
...
combined_res <- summarize_lc_model(
  car_suv_lc_7c,
  car_suv_lc_7c_apollo_probabilities,
  car_suv_lc_7c_apollo_inputs,
  database,
  "Combined",
  attributes,
  num_vars,
...
gt_car_suv_lc_7c <- gt_formatted %>%
  ...
  tab_header(
    title    = md("**BEV Battery Information Valuation: Latent Class Profile Summary**"),
...
gtsave(
  ...
  file = here::here(
    ...
    "battery_analysis",
    "apollo",
    "0_class_profile_summary_combined_lc_7c_rangeloss.html"
  )
)

# ============================================================
# CLASS 7 SUBGROUP ANALYSIS
# Hypothesis: C7 contains two distinct groups:
...
class_probs_7c <- read_parquet(here(
  "code",
  "output",
  "model_output",
  "battery_analysis",
  "apollo",
  "0_Combined_7c_class_probabilities.parquet"
)) %>%
  left_join(
...
# --- 5. Class 7 respondent-level dataset ---
c7_resp <- class_probs_7c %>%
  filter(prob_class_assign == "class7") %>%
  left_join(timing_c7, by = "psid") %>%
...
median_sec <- median(c7_resp$mean_cbc_sec, na.rm = TRUE)
...
diag_summary <- c7_resp %>%
  group_by(speed_group) %>%
  summarise(
    n                = n(),
    mean_duration_sec = weighted.mean(mean_cbc_sec, prob_class7, na.rm = TRUE),
    optout_rate      = weighted.mean(optout_rate_resp, prob_class7, na.rm = TRUE),
    optout_never_pct = weighted.mean(optout_never, prob_class7, na.rm = TRUE),
    .groups = "drop"
  )
...
compare_num <- map_dfr(num_vars, function(v) {
  map_dfr(c("Fast", "Slow"), function(grp) {
    sub <- c7_resp %>% filter(speed_group == grp)
    tibble(
      variable    = v,
      speed_group = grp,
      value       = weighted.mean(sub[[v]], sub$prob_class7, na.rm = TRUE)
    )
  })
}) %>%
...
compare_cate <- map_dfr(binary_cate_vars, function(v) {
  map_dfr(c("Fast", "Slow"), function(grp) {
    ...
    tibble(
      variable    = v,
      speed_group = grp,
      value       = weighted.mean(as.numeric(sub[[v]] == 1), sub$prob_class7, na.rm = TRUE)
    )
  })
}) %>%
  pivot_wider(names_from = speed_group, values_from = value)
...
# --- 9. GT table ---
gt_c7_subgroup <- display_tbl %>%
  group_by(section) %>%
  gt(rowname_col = "label") %>%
  tab_header(
    title    = md("**Class 7 Internal Subgroup Comparison: Fast vs. Slow Responders**"),
    subtitle = md(glue(
  ...
  tab_footnote(
    footnote  = "All means probability-weighted by each respondent's posterior P(class 7). Behavioral rows use raw means.",
    locations = cells_title("subtitle")
  ) %>%
  tab_style(
...
gtsave(
  ...
  file = here::here(
    ...
    "battery_analysis",
    "apollo",
    "0_c7_subgroup_fast_vs_slow.html"
  )
)

# --- 10. Scatter: duration vs opt-out rate per C7 respondent ---
p_c7_scatter <- c7_resp %>%
  mutate(
    mean_cbc_cap = pmin(
      mean_cbc_sec,
  ...
  annotate(
    ...
    hjust = 0, color = "firebrick", size = 3.2,
    family = "Roboto Condensed"
  ) +
  scale_color_gradient(low = "#FFCDD2", high = "#B71C1C", name = "P(class 7)") +
  scale_size_continuous(range = c(1, 4), guide = "none") +
  scale_y_continuous(labels = scales::percent, breaks = seq(0, 1, 0.2)) +
  labs(
    title    = "C7 Respondents: Task Duration vs. Opt-Out Rate",
...
ggsave(
  here(
    ...
    "images",
    "battery_analysis",
    "c7_duration_vs_optout_scatter.png"
  ),
  p_c7_scatter,
  width  = 9,
  height = 5.5,
