...
The initial sample comprised 3,657 respondents from across the United States. We applied multiple data quality screens to identify and remove low-quality responses, including speeders, respondents who failed the embedded attention check, those with internal inconsistencies across survey sections, and those who provided uninformative or incoherent text in open-ended responses. 

For the discrete choice experiment in particular, we applied an additional task-level speeder screen designed to identify inattentive responders without penalizing respondents whose short completion times reflected genuine category rejection. Following \textcite{greszkiImpactSpeedingData2015}, who show that relative response-time cutoffs based on the sample median outperform absolute thresholds for identifying inattentive online-panel respondents, we computed the median decision time across respondents for each of the six choice tasks and defined a task-level cutoff at one third of that median. Respondents who selected the no-choice option on all six tasks were exempt from this screen, since their short DCE duration plausibly reflects rejection of the BEV category rather than satisficing. Among the remaining respondents, those whose decision time fell below the one-third-median cutoff on three or more of the six tasks were classified as DCE speeders and removed from the analytic sample.

The final analytic sample consists of 2,916 respondents, yielding 17,496 choice observations across six tasks. Figure~\ref{fig:map_all} shows the geographic distribution of respondents by ZIP code; point size and color intensity are scaled to respondent count, indicating broad national coverage.
