...
car_suv_lc_6c <- readRDS(here(
  ...
  "battery_analysis",
  "apollo",
  "removebadcase_piecewise_rangeloss_car_suv_lc_6c_1_model.rds"
))
...
speeders_psid <- dur$psid[rowSums(below, na.rm = TRUE) > 2]

data_model <- data_model %>%
  filter(!psid %in% speeders_psid)

# Restrict to respondents actually used to fit the model (model's filter may be stricter)
model_fit_respID <- unique(car_suv_lc_6c_apollo_inputs$database$respID)
data_model <- data_model %>% filter(respID %in% model_fit_respID)
...
estimates_6c <- read_csv(here(
  ...
  "battery_analysis",
  "apollo",
  "removebadcase_piecewise_rangeloss_car_suv_lc_6c_1_estimates.csv"
))
  ...
  write_parquet(
    ...
    here(
      ...
      paste0(
        ...
        "_",
        n_classes,
        "c_class_probabilities_removebadcase.parquet"
      )
    )
  )
...
# ── Share of respondents by number of opt-outs (0–6) ─────────────────────────
{
  ...
  cp_optout <- read_parquet(here(
    ...
    "battery_analysis",
    "apollo",
    paste0("0_Combined_", n_k, "c_class_probabilities_removebadcase.parquet")
  )) %>%
...
# Re-run 3b_survey_duration_6c.R first whenever duration results are updated.
{
  duration_raw <- read_parquet(here(
    ...
    "battery_analysis",
    "apollo",
    "0_survey_duration_by_class_6c_removebadcase.parquet"
  ))
...
gtsave(
  ...
  file = here::here(
    ...
    "battery_analysis",
    "apollo",
    "0_class_profile_summary_combined_lc_6c_rangeloss_removebadcase.html"
  )
)
