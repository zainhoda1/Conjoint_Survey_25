...
# hist(a$duration_q1[a$duration_q1 < 300], breaks = 100)
# summary(a$duration_q1)
# summary(a$duration_q2)
# # n_distinct(data_model$psid)

# ---- Speeder removal: 1/4-median rule (Greszki et al. 2015) ----
# Always keep respondents who opted out on all six tasks; their short
# duration reflects category rejection, not satisficing.
# summary(data_model$duration_battery_dce)

optout_all_psid <- data_model %>%
  distinct(psid, qID, choice) %>%
  group_by(psid) %>%
  summarise(all_optout = all(choice == 4), .groups = "drop") %>%
  filter(all_optout) %>%
  pull(psid)
length(optout_all_psid)

duration_vars <- c(paste0("duration_q", 1:6), "duration_battery_dce")

dur <- data_model %>%
  distinct(psid, .keep_all = TRUE) %>%
  filter(!psid %in% optout_all_psid) %>%
  select(psid, all_of(duration_vars))

# 1/3-median cutoff per question and for total DCE time
cutoffs <- sapply(duration_vars, function(v) median(dur[[v]], na.rm = TRUE) / 3)

# Per-task speeder flags (below 1/3-median cutoff on each question)
q_cols <- paste0("duration_q", 1:6)
below <- sapply(q_cols, function(v) dur[[v]] < cutoffs[v])

# Speeders: under the 1/3-median cutoff on ALL six choice tasks
speeders_psid <- dur$psid[rowSums(below, na.rm = TRUE) > 1]

cat("1/3-median cutoffs (seconds):\n")
print(round(cutoffs, 1))
cat(sprintf(
  "\nAll-optout respondents (kept): %d\nSpeeders under cutoff on all 6 tasks: %d\n",
  length(optout_all_psid),
  length(speeders_psid)
))

data_model <- data_model %>%
  filter(!psid %in% speeders_psid)

# write data_model as a parquest
write_parquet(
  data_model,
  here(
    "data",
...
### Car+SUV
apollo_control = list(
  modelName = paste0("removebadcase_piecewise_rangeloss_car_suv_lc_6c_", i),
...
apollo_beta = c(
  ...
  # Class F is the reference (fixed to 0)
  delta_a = 0,
...
saveRDS(
  ...
  file = here(
    ...
    "battery_analysis",
    "apollo",
    "removebadcase_car_suv_lc_6c_apollo_inputs_piecewise_rangeloss.rds"
  )
)
...
saveRDS(
  ...
  file = here(
    ...
    "battery_analysis",
    "apollo",
    "removebadcase_car_suv_lc_6c_apollo_probabilities_piecewise_rangeloss.rds"
  )
)
