...
data_full <- read_parquet(here(
  "data", "dynata_prolific_joint", "data_joint.parquet"
))
...
profile_vars <- c(
  "gender", "hh_income", "education", "housing_tenure", "charger_access",
  "attitudes_1_a_ev_environment", "attitudes_2_a_ev_battery_environment",
  "attitudes_2_b_price_sensitive", "attitudes_2_b_risk_taker",
...
profile_summary <- data_analysis %>%
  ...
  summarise(
    ...
    pct_high_income        = mean(hh_income %in% c(
      "100k_150k", "150k_200k", "200k_plus"),                 na.rm = TRUE),
    pct_college_plus       = mean(education %in% c(
      "bachelors", "graduate"),                               na.rm = TRUE),
    pct_homeowner          = mean(housing_tenure == "own",    na.rm = TRUE),
    pct_charger_access     = mean(charger_access == TRUE |
                                    charger_access == "yes",  na.rm = TRUE),
    mean_ev_environ        = mean(attitudes_1_a_ev_environment,          na.rm = TRUE),
    mean_batt_environ      = mean(attitudes_2_a_ev_battery_environment,  na.rm = TRUE),
    ...
    pct_open_new_bev       = mean(next_veh_fuel_new_bev  == TRUE |
                                    next_veh_fuel_new_bev  == 1, na.rm = TRUE),
    pct_open_used_bev      = mean(next_veh_fuel_used_bev == TRUE |
                                    next_veh_fuel_used_bev == 1, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
gt_allout <- rate_summary %>%
  ...
  cols_label(
    n_total_Dynata   = "N", n_allout_Dynata   = "All-Optout", rate_Dynata   = "%",
    n_total_Prolific = "N", n_allout_Prolific = "All-Optout", rate_Prolific = "%",
    n_total_Overall  = "N", n_allout_Overall  = "All-Optout", rate_Overall  = "%"
  ) %>%
  tab_header(
    title    = md("**All-Optout Rate by Information Treatment and Platform**"),
    subtitle = md(paste0(
      "Respondents choosing no-choice in all 6 tasks  |  ",
      "Chi-sq (treatment): p = ", round(chisq_treat$p.value, 3), "  |  ",
      "Chi-sq (platform): p = ",  round(chisq_platform$p.value, 3)
    ))
  ) %>%
  tab_footnote(
