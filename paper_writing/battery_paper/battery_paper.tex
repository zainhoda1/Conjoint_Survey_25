...
The survey was administered online through two widely-used online research platforms. The data collection took place between December 2025 and April 2026. After initial data cleaning, we ended with 2,113 participants across the US. The geographic distribution of respondents is shown in Figure~\ref{fig:map_all}. Each point represents a ZIP code, with color and size scaled to respondent count. The median survey time is around 14 minutes. The survey participants were compensated through the research platforms.

\begin{figure}[pos=H]
    \centering
    \includegraphics[width=0.85\textwidth]{../../code/output/images/map_all.png}
...
\section{Method}
\subsection{Mixed Logit Model (MXL)}

The Mixed Logit (MXL) model relaxes the restrictive Independence of Irrelevant Alternatives (IIA) assumption of the standard MNL by allowing preference parameters to vary continuously across individuals \parencite{mcfaddenMixedMNLModels2000, trainDiscreteChoiceMethods2009}. We estimate the model directly in \textit{WTP space} \parencite{trainWTPSpacePreference2005}. Partitioning the attribute vector into price $p_{nti}$ and non-price attributes $\mathbf{x}_{nti}$, the WTP-space utility for respondent $n$ choosing alternative $i$ in choice scenario $t$ is:

\begin{equation}
    U_{nti} = -\alpha_n \bigl(p_{nti} - \mathbf{w}_n' \mathbf{x}_{nti}\bigr) + \varepsilon_{nti}
    \label{eq:mxl_utility}
\end{equation}

\noindent where $\alpha_n > 0$ is the individual-specific marginal disutility of price (typically log-normally distributed to enforce the correct sign), $\mathbf{w}_n$ is the vector of individual WTP values drawn from a mixing distribution $f(\mathbf{w} \mid \boldsymbol{\mu}, \boldsymbol{\Sigma})$ with population mean $\boldsymbol{\mu}$ and covariance $\boldsymbol{\Sigma}$, and $\varepsilon_{nti}$ is an IID Type I Extreme Value error. Because $\alpha_n$ and $\mathbf{w}_n$ are unobserved, the choice probability is obtained by integrating the logit kernel over the joint mixing distribution:

\begin{equation}
    P_{nti}(\boldsymbol{\mu}, \boldsymbol{\Sigma}) = \int\!\int \frac{\exp\!\left(-\alpha\bigl(p_{nti} - \mathbf{w}'\mathbf{x}_{nti}\bigr)\right)}{\displaystyle\sum_{j} \exp\!\left(-\alpha\bigl(p_{ntj} - \mathbf{w}'\mathbf{x}_{ntj}\bigr)\right)} \, f(\mathbf{w} \mid \boldsymbol{\mu}, \boldsymbol{\Sigma})\, g(\alpha) \, d\mathbf{w}\, d\alpha
    \label{eq:mxl_prob}
\end{equation}

\noindent This integral is evaluated by simulation using Halton draws \parencite{trainHaltonSequencesNumerical1999}. Accounting for the panel structure of six repeated choice tasks per respondent, the simulated log-likelihood is:

\begin{equation}
    \mathcal{L}(\boldsymbol{\mu}, \boldsymbol{\Sigma}) = \sum_{n=1}^{N} \ln \left[ \frac{1}{R} \sum_{r=1}^{R} \prod_{t=1}^{6} \frac{\exp\!\left(-\alpha_r\bigl(p_{nti_{nt}^*} - \mathbf{w}_r'\mathbf{x}_{nti_{nt}^*}\bigr)\right)}{\displaystyle\sum_{j} \exp\!\left(-\alpha_r\bigl(p_{ntj} - \mathbf{w}_r'\mathbf{x}_{ntj}\bigr)\right)} \right]
    \label{eq:mxl_ll}
\end{equation}

\noindent where $R$ is the number of Halton draws, $(\alpha_r, \mathbf{w}_r)$ is the $r$-th draw from the joint mixing distribution, and $i_{nt}^*$ is the alternative chosen by respondent $n$ in scenario $t$. Because estimation is carried out directly in WTP space, the population mean WTP for each non-price attribute $k$ is obtained directly as $\hat{\mu}_k$ from the estimated distribution of $\mathbf{w}_n$, without requiring any post-estimation ratio transformation.
...
\noindent where $\mathbf{X}_{nti}$ is a vector of observed BEV attributes presented in scenario $t$ to respondent $n$, including vehicle mileage, battery refurbishment status, estimated driving range and battery SOH currently (at years 3) and in the future (at year 8; 5 years from now), and purchase price; $\boldsymbol{\beta}_s$ is a vector of class-specific utility parameters capturing the marginal utility of each attribute for class $s$; and $\varepsilon_{nti \mid s}$ is a random error term assumed to be independently and identically distributed (IID) according to a Type I Extreme Value (Gumbel) distribution. Under this assumption, the conditional probability that respondent $n$ in class $s$ chooses alternative $i$ in scenario $t$ follows the multinomial logit (MNL) form:
...
The optimal number of latent classes is determined by estimating models with two through five classes and comparing fit using multiple information criteria: the log-likelihood (LL), Akaike Information Criterion (AIC), Bayesian Information Criterion (BIC), and class size. Lower values of AIC, BIC, and CAIC indicate better model fit penalized for complexity. The preferred specification is selected based on the lowest BIC, acceptable entropy, and substantively interpretable class profiles of reasonable size.
