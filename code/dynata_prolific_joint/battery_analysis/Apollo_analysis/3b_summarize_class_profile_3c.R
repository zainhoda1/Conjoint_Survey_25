...
car_suv_lc_3c <- readRDS(here(
  ...
  "piecewise_rangeloss_car_suv_lc_3c_1_model.rds"
))
car_suv_lc_3c_apollo_inputs <- readRDS(here(
  "code", "output", "model_output", "battery_analysis", "apollo",
  "car_suv_lc_3c_apollo_inputs_piecewise_rangeloss.rds"
))
car_suv_lc_3c_apollo_probabilities <- readRDS(here(
  "code", "output", "model_output", "battery_analysis", "apollo",
  "car_suv_lc_3c_apollo_probabilities_piecewise_rangeloss.rds"
))
...
estimates_3c <- read_csv(here(
  "code", "output", "model_output", "battery_analysis", "apollo",
  "piecewise_rangeloss_car_suv_lc_3c_1_estimates.csv"
))
colnames(estimates_3c)[1] <- "variable"

estimates_long <- estimates_3c %>%
  as.data.frame() %>%
  select(1:2, 6, 8) %>%
...
summarize_lc_model <- function(model, apollo_probabilities, apollo_inputs,
  ...
  num_summary <- map_dfr(num_vars, function(v) {
    map_dbl(1:n_classes, function(k) {
      weighted.mean(db_indiv[[v]], db_indiv[[paste0("prob_class", k)]], na.rm = TRUE)
  ...
  combined %>% rename_with(~ paste0(model_tag, "_", .), starts_with("class"))
}
...
combined_res <- summarize_lc_model(
  car_suv_lc_3c, car_suv_lc_3c_apollo_probabilities, car_suv_lc_3c_apollo_inputs,
...
fmt_row <- function(x, fmt) {
  if (fmt == "dollar") dollar(x, accuracy = 0.1)
  else if (fmt == "pct") percent(x, accuracy = 0.1)
  else number(x, accuracy = 0.1)
}

formatted <- combined_all %>%
  rowwise() %>%
  mutate(
    fmt = case_when(variable %in% attributes_long ~ "dollar", grepl("_share$", variable) ~ "pct", TRUE ~ "number"),
...
summarize_class_size <- function(model, apollo_probabilities, apollo_inputs) {
  ...
  conditionals %>%
    ...
    ungroup() %>%
    rowwise() %>%
    mutate(class_size = round(class_size * mean_probability, 0),
...
gt_car_suv_lc_3c <- gt_formatted %>%
  ...
  tab_footnote(footnote = "WTP significance bounded by the less significant of (attribute, price): stars reflect max(p_attr, p_price).", locations = cells_title("subtitle")) %>%
...
gtsave(gt_car_suv_lc_3c,
  file = here::here("code","output","model_output","battery_analysis","apollo",
                    "0_class_profile_summary_combined_lc_3c_rangeloss.html"))

# ── Class 3 Subgroup Analysis ─────────────────────────────────────────────────
library(glue); library(lubridate)

class_probs_3c <- read_parquet(here(
  "code","output","model_output","battery_analysis","apollo",
...
optout_per_resp <- database %>%
  select(psid, qID, choice) %>% distinct() %>% group_by(psid) %>%
...
c3_resp <- class_probs_3c %>%
  filter(prob_class_assign == "class3") %>%
  left_join(timing_c3, by = "psid") %>%
  left_join(optout_per_resp, by = "psid") %>%
  left_join(resp_covariates, by = "psid") %>%
  filter(!is.na(mean_cbc_sec))

median_sec <- median(c3_resp$mean_cbc_sec, na.rm = TRUE)
c3_resp <- c3_resp %>%
  mutate(speed_group = factor(if_else(mean_cbc_sec <= median_sec, "Fast", "Slow"), levels = c("Fast","Slow")))

diag_summary <- c3_resp %>% group_by(speed_group) %>%
  summarise(n = n(),
    mean_duration_sec = weighted.mean(mean_cbc_sec, prob_class3, na.rm = TRUE),
    optout_rate       = weighted.mean(optout_rate_resp, prob_class3, na.rm = TRUE),
    optout_never_pct  = weighted.mean(optout_never, prob_class3, na.rm = TRUE), .groups = "drop")

compare_num <- map_dfr(num_vars, function(v) {
  map_dfr(c("Fast","Slow"), function(grp) {
    sub <- c3_resp %>% filter(speed_group == grp)
    tibble(variable = v, speed_group = grp,
           value = weighted.mean(sub[[v]], sub$prob_class3, na.rm = TRUE))
  })
...
compare_cate <- map_dfr(binary_cate_vars, function(v) {
  map_dfr(c("Fast","Slow"), function(grp) {
    sub <- c3_resp %>% filter(speed_group == grp)
    tibble(variable = v, speed_group = grp,
           value = weighted.mean(as.numeric(sub[[v]] == 1), sub$prob_class3, na.rm = TRUE))
  })
}) %>% pivot_wider(names_from = speed_group, values_from = value)
...
compare_all <- bind_rows(compare_num, compare_cate) %>%
  ...
  mutate(label = coalesce(label, variable), fmt = coalesce(fmt, "number"), Diff = Slow - Fast,
    across(c(Fast, Slow, Diff), list(fmt = ~ case_when(
      fmt == "dollar" ~ dollar(.x, accuracy = 0.1),
      fmt == "pct"    ~ percent(.x, accuracy = 0.1),
      TRUE            ~ number(.x, accuracy = 0.1))), .names = "{col}_fmt")) %>%
...
gt_c3_subgroup <- display_tbl %>%
  group_by(section) %>% gt(rowname_col = "label") %>%
  tab_header(
    title    = md("**Class 3 Internal Subgroup Comparison: Fast vs. Slow Responders**"),
  ...
  tab_footnote(footnote = "All means probability-weighted by each respondent's posterior P(class 3). Behavioral rows use raw means.", locations = cells_title("subtitle")) %>%
...
gtsave(gt_c3_subgroup,
  file = here::here("code","output","model_output","battery_analysis","apollo",
                    "0_c3_subgroup_fast_vs_slow.html"))

p_c3_scatter <- c3_resp %>%
  mutate(mean_cbc_cap = pmin(mean_cbc_sec, quantile(mean_cbc_sec, 0.97, na.rm = TRUE))) %>%
  ggplot(aes(x = mean_cbc_cap, y = optout_rate_resp, color = prob_class3, size = prob_class3)) +
  geom_point(alpha = 0.7) +
  geom_vline(xintercept = median_sec, linetype = "dashed", color = "firebrick", linewidth = 0.7) +
  annotate("text", x = median_sec + 1, y = 0.95,
           label = glue("median = {round(median_sec, 1)}s"), hjust = 0,
           color = "firebrick", size = 3.2, family = "Roboto Condensed") +
  scale_color_gradient(low = "#FFCDD2", high = "#B71C1C", name = "P(class 3)") +
  scale_size_continuous(range = c(1, 4), guide = "none") +
  scale_y_continuous(labels = scales::percent, breaks = seq(0, 1, 0.2)) +
  labs(title = "C3 Respondents: Task Duration vs. Opt-Out Rate",
...
ggsave(here("code","output","images","battery_analysis","c3_duration_vs_optout_scatter.png"),
       p_c3_scatter, width = 9, height = 5.5, dpi = 300)
